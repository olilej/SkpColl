...
bool IntrLine2Line2<Real>::Find ()
{
    ...
    {
        mQuantity = 1;
        mPoint = mLine0->Origin + parameter[0]*mLine0->Direction;
    }
    else if (mIntersectionType == IT_LINE)
    {
        mQuantity = INT_MAX;
...
//----------------------------------------------------------------------------
template <typename Real>
int IntrLine2Line2<Real>::Classify (Real* s, Vector2<Real>* diff,
    Vector2<Real>* diffN)
{
    ...
    Vector2<Real> originDiff = mLine1->Origin - mLine0->Origin;
    if (diff)
    {
        *diff = originDiff;
    }

    Real D0DotPerpD1 = mLine0->Direction.DotPerp(mLine1->Direction);
    if (Math<Real>::FAbs(D0DotPerpD1) > Math<Real>::ZERO_TOLERANCE)
    {
        ...
        if (s)
        {
            ...
            Real diffDotPerpD0 = originDiff.DotPerp(mLine0->Direction);
            Real diffDotPerpD1 = originDiff.DotPerp(mLine1->Direction);
    ...
    Real diffNDotPerpD1 = originDiff.DotPerp(mLine1->Direction);
