...
//----------------------------------------------------------------------------
template <typename Real>
NaturalSpline3<Real>::NaturalSpline3 (int numSegments, Real* times,
    Vector3<Real>* points, const Vector3<Real>& derivativeStart,
...
Vector3<Real> NaturalSpline3<Real>::GetFirstDerivative (Real t) const
{
    ...
    Real dt;
    GetKeyInfo(t, key, dt);
    return mB[key] + dt*(((Real)2)*mC[key] + ((Real)3)*dt*mD[key]);
}
//----------------------------------------------------------------------------
template <typename Real>
Vector3<Real> NaturalSpline3<Real>::GetSecondDerivative (Real t) const
{
    ...
    Real dt;
    GetKeyInfo(t, key, dt);
    return ((Real)2)*mC[key] + ((Real)6)*dt*mD[key];
}
//----------------------------------------------------------------------------
template <typename Real>
